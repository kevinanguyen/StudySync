...
export default function RightPanel() {
  ...
  const filteredFriends: Friend[] = (FRIENDS as Friend[]).filter((f: Friend) => f.name.toLowerCase().includes(lowerSearch));
  const filteredGroups: Group[] = (GROUPS_DATA as Group[]).filter((g: Group) => g.name.toLowerCase().includes(lowerSearch));
        ...
        <div className="px-3 pt-2 pb-3">
          ...
          <div className="flex flex-col gap-0.5">
            {displayedFriends.map((friend: Friend) => {
              const cfg = statusConfig[friend.status as keyof typeof statusConfig] || statusConfig.available;
              return (
